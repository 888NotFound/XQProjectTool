...
@implementation NSString (XQCRC)

#pragma mark -- 文档的

- (NSString *)lmCrc16 {
        //调用字符串示例: 41 54 08 14 57 00 01 01 01, 两位为一个十六进制, 空格可以不要
        // 结果是: DABC
    NSString *str = [self stringByReplacingOccurrencesOfString:@" " withString:@""];
    NSUInteger length = str.length/2;
    
    Byte b[length];
    for (int i = 0; i < str.length/2; i++) {
        int a = i * 2;
            // 因为传进来的都是16进制, 而这边的转的规则是传10进制的数字进去, 所以, 这里先把16进制的数值都转为10进制
        NSString *hexStr = [str substringWithRange:NSMakeRange(a, 2)];
        hexStr = [hexStr xq_hexadecimalTransferDecimal];
        b[i] = (unsigned char)hexStr.integerValue;
    }
    unsigned short result = crc16Sum(b, str.length/2);
    return [NSString toHexInt:result];
}

// 其实..这个好像是 8把, 因为下面都是左移8或者右移8, 不懂...
static inline unsigned short crc16Sum(unsigned char *q,unsigned char len) {
    unsigned short crc = 0;
    
        // Android那边拿过来的, 经测试, 和下面那个一样意思, 只是这个分开步骤而已
//    unsigned short hb = 0;
//    int index;
//    while (len-- > 0) {
//        hb = (crc / 256); //以8位二进制数的形式暂存CRC的高8位
//        index = ((hb ^ *q++) & 0xff); //求得所查索引下标
//        crc <<= 8; // 左移8位，相当于CRC的低8位乘以
//        crc ^= (crc_table[index]); // 高8位和当前字节相加后再查表求CRC ，再加上以前的CRC
//    }
//
//    return crc;
...
//
//
//- (unsigned short)otherCrc16 {
//    return [NSData crc16:(Byte *)self.bytes andLength:self.length];
//}
//
//    //第一种高低位的crc16校验
...
//+ (unsigned short)crc16:(unsigned char *)puchMsg andLength:(unsigned short)usDataLen
//
//{
...
/*
- (uint16_t)getCrc16 {
    return [NSData crc16:(Byte *)self.bytes length:self.length];
}
